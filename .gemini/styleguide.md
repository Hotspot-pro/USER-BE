...
---
## 0) 언어 규칙 (중요)
- Gemini Code Assist의 모든 PR 요약/리뷰/후속 답변은 **한국어로만** 작성한다.
- 단, 아래 항목은 **원문(영문) 그대로 유지**한다: 코드 식별자(클래스/메서드/변수명), 라이브러리/프레임워크명, 에러 메시지/로그, 설정 키, API 경로.
- 설명 문장은 영어로 쓰지 않는다. (필요 시 readOnly, N+1 같은 “단어 수준”만 허용)
